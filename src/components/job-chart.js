export class JobChart {
  ...
  constructor(selector) {
    ...
    if (this.container.__resizeObserver) {
      ...
      this.container.__resizeObserver = null;
    }

    this.data = null;
    this.activeIdx = null; // Default active category for detail highlighted
  }
  ...
  draw() {
    ...
    if (this.activeIdx === null) {
      comparisonBox.innerHTML = `
        <div style="font-size:0.85rem; color:var(--text-secondary); padding: 1rem 0; text-align:center;">
          Click on a job category above for more details.
        </div>
      `;
    } else {
